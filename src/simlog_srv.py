...
class MyParser:
	...
	def check_format_info(self, info):
		...
		if "date" not in info:
			...
			info["date"] = 'date "+%4d:%02d:%02d:_____:%02d:%02d:"' % (now.year, now.month, now.day, now.hour, now.minute);
	...
	def parse(self, xml_string):
		...
		try:
			...
			print "ERROR: While parsing XML! '%s'" % xml_string;
			return None;
	...
	def handleStartElement(self, name, attrs):
		if not self.re_sysdot.match(name):
			self.tag = name;
		else:
			self.tag = None;
...
def htm_table(program, user):
	global ignore_list, all_receives;
	str = table_header;
	all_columns = {};
	## regular expr
	re_prog = re.compile(program);
	re_user = re.compile(user);
	##
	for r in all_receives:
		if re_prog.search(r["program"]) and re_user.search(r["user"]):
			for idx in r:
	...
	for r in all_receives:
		if re_prog.search(r["program"]) and re_user.search(r["user"]):
			str += """<tr class="%s" class="%s" class="%s"><td>%s</td><td>%s</td><td>%s</td>""" % (
					r["clsdate"], r["clsprogram"], r["user"],
					r["user"], r["program"], r["date"]
				);
			for c in all_columns:
				if c in r:
					str += "<td>%s</td>" % r[c];
...
def htm_page(program, file, user):
#	try:
	if True:
		f = open(file, "w");
		f.write(htm_header);
		f.write("<h1>'%s' script log utility</h1>" % program);
		if False:
			f.write(htm_form);
		f.write(htm_table(program, user));
		f.write(htm_footer);
#	except:
...
def listen_update():
	...
	while not finish:
		...
		if not obj:
			...
			save_me(obj["save_me"]);
		elif "print_html" in obj:
			if "user" not in obj:
				obj["user"] = ".*";
			print "%s : print_html to %s, user %s" % (datetime.datetime.now(), obj["file"], obj["user"]);
			htm_page(obj["print_html"], obj["file"], obj["user"]);
		elif "quit" in obj:
			today = datetime.date.today();
			save_f = "./save_file_%02d%02d%04d.pkl" % (today.day, today.month, today.year);
			print "%s : Default save to %s" % (datetime.datetime.now(), save_f);
			save_me(save_f);
			print "%s : quit" % (datetime.datetime.now());
			finish = True;
		else:
			all_receives.append(obj);
		sys.stdout.flush();
